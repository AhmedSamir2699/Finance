<x-app-layout>
    ...
    <div class="container mt-4 mx-auto">
        ...
        <form method="POST" action="{{ route('finance-items.update',$financeItem) }}" enctype="multipart/form-data"
            class="grid grid-cols-1 md:grid-cols-2 gap-4 bg-white p-6 rounded-md shadow-md">
            @csrf

            {{-- Amount --}}
            <div class="col-span-1">
                <label for="name" class="block text-sm font-medium text-slate-700 py-2">
                    {{ trans('cruds.financeItem.fields.name') }} <span class="text-red-500">*</span>
                </label>
                <input type="text" name="name" id="name" step="0.01"
                    class="appearance-none rounded border border-gray-300 px-4 py-2 w-full text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 @error('name') border-red-500 @enderror"
                    value="{{ old('name', $financeItem->name) }}"
                    required>
                @error('name')
                    <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
                @enderror
                <p class="text-xs text-gray-500 mt-1">{{ trans('cruds.financeItem.fields.name_helper') }}</p>
            </div>

            {{-- Amount --}}
            <div class="col-span-1">
                <label for="amount" class="block text-sm font-medium text-slate-700 py-2">
                    {{ trans('cruds.financeItem.fields.amount') }} <span class="text-red-500">*</span>
                </label>
                <input type="number" name="amount" id="amount" step="0.01"
                    ...
                    value="{{ old('amount', $financeItem->amount) }}"
                    required>
                @error('amount')
                    <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
                @enderror
                <p class="text-xs text-gray-500 mt-1">{{ trans('cruds.financeItem.fields.amount_helper') }}</p>
            </div>

            <div class="col-span-1">
                <label for="parent_id" class="block text-sm font-medium text-slate-700 py-2">{{ trans('cruds.financeItem.fields.parent') }}</label>
                <select class="appearance-none rounded border border-gray-300 px-4 py-2 w-full text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 @error('parent_id') border-red-500 @enderror" name="parent_id"
                    ...
                    @foreach ($parents as $id => $entry)
                        <option value="{{ $id }}" {{ old('parent_id',$financeItem->parent_id) == $id ? 'selected' : '' }}>
                            {{ $entry }}</option>
                    @endforeach
                </select>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.financeItem.fields.parent_helper') }}</span>
            </div>
