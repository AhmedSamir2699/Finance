<x-app-layout>
    ...
    <div class="container mt-4 mx-auto">
        {{-- <div class="bg-white py-2 px-4 rounded-md shadow-md border" x-data="{ 
            ...
            <form method="POST" action="{{ route('income-categories.update',['incomeCategory'=>$incomeCategory]) }}" enctype="multipart/form-data"
    ...
    <div class="col-span-2">
        <label for="name" class="block text-sm font-medium text-slate-700 py-2">
            {{ trans('cruds.incomeCategory.fields.name') }} <span class="text-red-500">*</span>
        </label>
        <input type="text" name="name" id="name"
            class="appearance-none rounded border border-gray-300 px-4 py-2 w-full text-sm placeholder-gray-400 text-gray-700 focus:outline-none focus:ring-2 focus:ring-primary-500 @error('name') border-red-500 @enderror"
            value="{{ old('name',$incomeCategory->name ) }}" required>
        @error('name')
            <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
        @enderror
        <p class="text-xs text-gray-500 mt-1">{{ trans('cruds.incomeCategory.fields.name_helper') }}</p>
    </div>
