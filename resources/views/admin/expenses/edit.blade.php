        ...
        {{-- <div class="bg-white py-2 px-4 rounded-md shadow-md border" x-data="{ 
            ...
            <form method="POST" action="{{ route('expenses.update',$expense) }}" enctype="multipart/form-data"
                ...
                <div class="col-span-2">
                    <label for="expense_category_id" class="block text-sm font-medium text-slate-700 py-2">
                        {{ trans('cruds.expense.fields.expense_category') }}
                    </label>
                    <select name="expense_category_id" id="expense_category_id"
                        class="appearance-none rounded border border-gray-300 px-4 py-2 w-full text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 @error('expense_category') border-red-500 @enderror">
                        @foreach($expense_categories as $id => $entry)
                            <option value="{{ $id }}" {{ old('expense_category_id',$expense->expense_category_id) == $id ? 'selected' : '' }}>{{ $entry }}</option>
                        @endforeach
                    </select>
                    @error('expense_category')
                        <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
                    @enderror
                    <p class="text-xs text-gray-500 mt-1">{{ trans('cruds.expense.fields.expense_category_helper') }}</p>
                </div>

                {{-- Entry Date --}}
                <div class="col-span-1">
                    <label for="entry_date" class="block text-sm font-medium text-slate-700 py-2">
                        {{ trans('cruds.expense.fields.entry_date') }} <span class="text-red-500">*</span>
                    </label>
                    ...
                    @error('entry_date')
                        <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
                    @enderror
                    <p class="text-xs text-gray-500 mt-1">{{ trans('cruds.expense.fields.entry_date_helper') }}</p>
                </div>

                {{-- Amount --}}
                <div class="col-span-1">
                    <label for="amount" class="block text-sm font-medium text-slate-700 py-2">
                        {{ trans('cruds.expense.fields.amount') }} <span class="text-red-500">*</span>
                    </label>
                    ...
                    @error('amount')
                        <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
                    @enderror
                    <p class="text-xs text-gray-500 mt-1">{{ trans('cruds.expense.fields.amount_helper') }}</p>
                </div>
                ...
                <div class="col-span-2">
                    ...
                    @error('description')
                        <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
                    @enderror
                    <p class="text-xs text-gray-500 mt-1">{{ trans('cruds.expense.fields.description_helper') }}</p>
                </div>

                {{-- Finance Item --}}
                <div class="col-span-2">
                    <label for="finance_item_id" class="block text-sm font-medium text-slate-700 py-2">
                        {{ trans('cruds.expense.fields.finance_item') }}
                    </label>
                    <select name="finance_item_id" id="finance_item_id"
                        ...
                        @foreach($finance_items as $id => $entry)
                            <option value="{{ $id }}" {{ old('finance_item_id',$expense->finance_item_id) == $id ? 'selected' : '' }}>{{ $entry }}</option>
                        @endforeach
                    </select>
                    @error('finance_item')
                        <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
                    @enderror
                    <p class="text-xs text-gray-500 mt-1">{{ trans('cruds.expense.fields.finance_item_helper') }}</p>
                </div>
