                ...
                <div class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
                    ...
                    <div class="mb-4">
                        <label class="block text-gray-700 text-sm font-bold mb-2" for="display_name">
                            {{ __('roles.edit.form.display_name') }}
                        </label>
                        <input
                            class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                            id="display_name" name="display_name" type="text" value="{{ $role->display_name }}">
                    </div>
                    <div class="mb-4">
                            ...
                            <select
                                ...
                                @foreach (\App\Models\Role::get() as $answerToRole)
                                    <option value="{{ $answerToRole->id }}"
                                        {{ ($role->superior && $answerToRole->id == $role->superior->id) ? 'selected' : '' }}>
